...
import { BarChart3, Calendar, Filter, Minus, Plus, RotateCw, Users } from "lucide-react";
...
import { fetchTeamLeaderProductivityReport } from "@/lib/services/team-leader";

export const Route = createFileRoute("/team-leader/productivity/staff")({
  head: () => ({
    meta: [
      { title: "Staff Productivity Index | Team Leader Portal" },
      { name: "description", content: "Telecaller calling metrics, conversion rates, and visit ratios." },
    ],
  }),
  component: TeamLeaderStaffProductivityPage,
});

export function TeamLeaderStaffProductivityPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetchTeamLeaderProductivityReport(
        startDate || undefined,
        endDate || undefined
      );
      setData(res.staff_data || []);
    } catch (err: any) {
      toast.error(err.message || "Failed to load productivity report.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
            <BarChart3 className="size-6 text-[#6732F2]" />
            Staff Productivity Index
          </h1>
          <p className="text-xs sm:text-sm text-gray-500">
            Analyze call volumes, lead generation efficiency, and visit conversion ratios per telecaller.
          </p>
        </div>
      </div>
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row sm:items-end gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Filter className="size-3.5" /> Filter Index
              </Button>
              <Button
      ...
      <Card className="border rounded-xl bg-white shadow-xs overflow-hidden">
        <CardHeader className="p-3 sm:p-4 border-b">
          <CardTitle className="text-sm sm:text-base font-bold text-gray-900">
            Telecallers Performance Benchmark ({data.length})
          </CardTitle>
          <CardDescription className="text-xs text-gray-500">
            Conversion statistics evaluated across total outbound dialing activity.
          </CardDescription>
        </CardHeader>
                ...
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="h-24 text-center text-xs text-gray-500">
                      No productivity data recorded for this date range.
                    </TableCell>
                  </TableRow>
                ) : (
                  data.map((row, idx) => (
