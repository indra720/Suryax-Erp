...
export const Route = createFileRoute("/team-leader/leads-report/pending-followups")({
  head: () => ({
    meta: [
      { title: "Pending Follow-ups Queue | Team Leader Portal" },
      { name: "description", content: "Overdue and pending customer follow-ups needing immediate callback." },
    ],
  }),
  component: TeamLeaderPendingFollowupsPage,
});

export function TeamLeaderPendingFollowupsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to load pending followups");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async () => {
    try {
      const payload: { status: string; start_date?: string; end_date?: string } = {
        status: "pending_followups",
      };
      if (startDate) payload.start_date = startDate;
      if (endDate) payload.end_date = endDate;
      await exportTeamLeaderLeads(payload);
      toast.success("Pending follow-up leads exported successfully.");
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500">
            Overdue customer leads that missed scheduled follow-up timestamps.
          </p>
        </div>
        <Button
        ...
        <CardHeader className="p-3 sm:p-4 pb-2 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <CardDescription className="text-xs text-gray-500">
              Urgent callback queue for the telecalling desk.
            </CardDescription>
          </div>
                    ...
                    <TableRow key={lead.id || idx} className="text-xs hover:bg-gray-50/70">
                      ...
                      <TableCell className="text-center">
                        <Button variant="ghost" size="icon" className="size-7 text-emerald-600" asChild>
                          <a
                            href={`https://wa.me/91${lead.call || lead.phone}?text=${encodeURIComponent(`Hello ${lead.name}, connecting with you from Vrindavan Real Estate.`)}`}
                            target="_blank"
                      ...
                      <TableCell className="hidden md:table-cell text-gray-600 max-w-xs truncate">
                        {lead.message || "Pending customer response"}
                      </TableCell>
