...
export const Route = createFileRoute("/team-leader/leads-report/interested")({
  head: () => ({
    meta: [
      { title: "Interested Hot Leads | Team Leader Portal" },
      { name: "description", content: "High intent prospective property buyers ready for visit and closing." },
    ],
  }),
  component: TeamLeaderInterestedReportPage,
});

export function TeamLeaderInterestedReportPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to load interested leads");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async () => {
    try {
      const payload: { status: string; start_date?: string; end_date?: string; all_interested?: string } = {
        status: "Intrested",
        all_interested: "1",
      };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500">
            High intent customers who have expressed interest in purchasing villas, plots, or apartments.
          </p>
        </div>
        <Button
        ...
        <CardHeader className="p-3 sm:p-4 pb-2 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <CardTitle className="text-sm sm:text-base font-bold text-gray-900">
              Verified Interested Prospects ({leads.length})
            </CardTitle>
            <CardDescription className="text-xs text-gray-500">
              Direct line to warm customers ready for site visit scheduling.
            </CardDescription>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 text-[11px] font-semibold text-gray-600 uppercase">
                  ...
                  <TableHead className="hidden md:table-cell">Requirement Notes</TableHead>
                ...
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-24 text-center text-xs text-gray-500">
                      No interested leads found in this period.
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    <TableRow key={lead.id || idx} className="text-xs hover:bg-gray-50/70">
                      ...
                      <TableCell className="text-center">
                        <Button variant="ghost" size="icon" className="size-7 text-blue-600" asChild>
                          <a href={`tel:${lead.call || lead.phone}`} title="Call Customer">
                      ...
                      <TableCell className="text-center">
                        <Button variant="ghost" size="icon" className="size-7 text-emerald-600" asChild>
                          <a
                            href={`https://wa.me/91${lead.call || lead.phone}?text=${encodeURIComponent(`Hello ${lead.name}, connecting regarding your property inquiry with Vrindavan Real Estate.`)}`}
                            target="_blank"
                      ...
                      <TableCell className="hidden md:table-cell text-gray-600 max-w-xs truncate">
                        {lead.message || "Customer interested in residential plots"}
                      </TableCell>
