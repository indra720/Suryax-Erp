...
export const Route = createFileRoute("/team-leader/leads-report/today-followups")({
  head: () => ({
    meta: [
      { title: "Today Follow-ups Queue | Team Leader Portal" },
      { name: "description", content: "Prospects scheduled for follow-up calling today." },
    ],
  }),
  component: TeamLeaderTodayFollowupsPage,
});

export function TeamLeaderTodayFollowupsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to load today followups");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async () => {
    try {
      const payload: { status: string; start_date?: string; end_date?: string } = {
        status: "today_followups",
      };
      if (startDate) payload.start_date = startDate;
      if (endDate) payload.end_date = endDate;
      await exportTeamLeaderLeads(payload);
      toast.success("Today follow-up leads exported successfully.");
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
            <Clock className="size-6 text-[#6732F2]" />
            Today Follow-ups Queue
          </h1>
          <p className="text-xs sm:text-sm text-gray-500">
            Prospects committed for telecalling updates, plot brochures, or visit confirmations today.
          </p>
        </div>
        <Button
        ...
        <CardHeader className="p-3 sm:p-4 pb-2 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <CardTitle className="text-sm sm:text-base font-bold text-gray-900">
              Today Leads List ({leads.length})
            </CardTitle>
            <CardDescription className="text-xs text-gray-500">
              Immediate action calling queue for today.
            </CardDescription>
          </div>
                    ...
                    <TableRow key={lead.id || idx} className="text-xs hover:bg-gray-50/70">
                      ...
                      <TableCell className="text-center">
                        <Button variant="ghost" size="icon" className="size-7 text-emerald-600" asChild>
                          <a
                            href={`https://wa.me/91${lead.call || lead.phone}?text=${encodeURIComponent(`Hello ${lead.name}, regarding your property inquiry today with Vrindavan Real Estate.`)}`}
                            target="_blank"
                      ...
                      <TableCell className="hidden md:table-cell text-gray-600 max-w-xs truncate">
                        {lead.message || "Customer requested follow-up today"}
                      </TableCell>
